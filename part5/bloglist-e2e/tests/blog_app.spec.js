...
describe('Blog app', () => {
  ...
  describe('When logged in', () => {
    ...
    test('blogs ordered by likes', async({page}) =>{
        await createBlog(page,'Bananas today','Wow Girl','www.eee')
        await createBlog(page,'Bananas tomorrow','Wow Girl','www.eeeaa')
        await expect(page.getByText('Bananas tomorrowWow Girl')).toBeVisible
        const blogsContainer = await page.getByTestId('blogsToShow')
        await expect((blogsContainer).locator('.Blog')).toHaveCount(2)
        const viewButtons = await blogsContainer.getByText('View').all()
        console.log(viewButtons)
        await viewButtons[1].click()
        await page.getByRole('button', { name: 'Like this post!' }).click()
        await expect(page.getByText('1')).toBeVisible()
        const blogs = await page.getByText('Wow Girl').all()
        await expect(blogs[0]).toContainText('Bananas tomorrowWow Girl')
    })
  })
})